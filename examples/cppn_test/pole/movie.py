...
def make_movie(net, force_function, duration_seconds, output_filename):
    ...
    cart = gz.rectangle(scale * 0.5, scale * 0.25, xy=(600, 80), stroke_width=1, fill=(0, 1, 0))
    pole = gz.rectangle(scale * 0.1, scale * 1.0, xy=(600, 55), stroke_width=1, fill=(1, 1, 0))
    ...
    def make_frame(t):
        ...
        # Draw pole.
        group = gz.Group((pole,)).translate((visX, 0)).rotate(sim.theta, center=(600 + visX, 80))
        group.draw(surface)

        return surface.get_npimage()

    clip = mpy.VideoClip(make_frame, duration=duration_seconds)
    clip.write_videofile(output_filename, codec="mpeg4", fps=60)