...
class MazeMaker:
    ...
    def __init__(self, rows: int, columns: int, obstacle_ratio: float, minimum_route_length: int,
                 seed: int = None) -> None:
        """
    ...
    def get_neighbours(self, location, grid):
        """
        This method returns a list of all valid neighbours for a given location.
        ...
        a = tuple(location + UP)
        if location[ROW] > 0 and grid[a] < -1:
            neighbours.append(a)

        a = tuple(location + DOWN)
        if location[ROW] < self.rows - 1 and grid[a] < -1:
            neighbours.append(a)

        a = tuple(location + LEFT)
        if location[COL] > 0 and grid[a] < -1:
            neighbours.append(a)

        a = tuple(location + RIGHT)
        if location[COL] < self.columns - 1 and grid[a] < -1:
            neighbours.append(a)

        return neighbours

    def get_neighbour_by_value(self, grid: np.ndarray, location, value: int) :
        """
        This method checks and returns the neighbouring locations for a given
        value (0: path or 1: wall).
        ...
        """
        a = tuple(location + UP)
        if location[ROW] > 0 and grid[a] == value:
            return a

        a = tuple(location + DOWN)
        if location[ROW] < self.rows - 1 and grid[a] == value:
            return a

        a = tuple(location + LEFT)
        if location[COL] > 0 and grid[a] == value:
            return a

        a = tuple(location + RIGHT)
        if location[COL] < self.columns - 1 and grid[a] == value:
            return a

    def explore_path(self, new_expand, grid) -> bool:
        """
        This method explores the grid to see if its solvable.
        ...
        """
        length = 0
        while len(new_expand) != 0:  # as long as there is something to expand
    ...
    def trace_route(self, grid, finish):
        """
        This method traces back the route of a solvable maze.
        ...
        while True:
            ...
            if length == 0:
                break
            route[:0] = [self.get_neighbour_by_value(grid, route[0], length - 1)]

        return route

    def find_route(self, start, finish, grid) :
        """
        This method checks wether a maze is solvable and returns the route.
