...
def getshape(
    ...
    Index, X, Y, Cat = find_contour(
        outputs, thresh, pcd, shapex, shapey,'sym4'
    )
    x_values = X.flatten()
    y_values = Y.flatten()
    ...
    plt.show()
    plt.savefig(f"./shapes/{genome.key}.png", transparent=True, dpi=600)
    plt.close()
...
def draw_shape(
    ...
    if is_mesh:
        ...
        if config.pcdtype == "parallel":
            ...
            #     plt.gca().set_axis_off()
            #     plt.show()
            #     plt.savefig(f".{to_path}_{f1}_{f2}_pcd.png", transparent=True)
            #     plt.close()
